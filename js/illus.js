...
export function compo(k, label = '') {
  ...
  switch (k) {
    ...
    case 'yeux-egalite': s += `<rect class="i-wall" x="0" y="0" width="160" height="90"/>${[40, 80, 120].map(x => `<rect class="i-wall2" x="${x - 12}" y="8" width="24" height="56"/><rect class="i-door" x="${x - 7}" y="14" width="14" height="50"/>`).join('')}<rect class="i-ground" x="0" y="64" width="160" height="26"/>${bust(40, 44, 6, 1)}${bust(80, 44, 6)}${bust(120, 44, 6, -1)}<line class="i-eyeline" x1="4" y1="43.4" x2="156" y2="43.4" ${NS}/><text class="i-txt i-cap" x="6" y="80">même hauteur</text>`; break;
    case 'yeux-dominant': s += `<rect class="i-wall" x="0" y="0" width="160" height="90"/><g opacity=".55">${[16, 38, 60, 82].map(x => bust(x, 50, 5)).join('')}</g><line class="i-eyeline" x1="0" y1="49.5" x2="92" y2="49.5" ${NS}/>${bust(126, 26, 13, -1, 'i-fig')}<line class="i-eyeline" x1="0" y1="24.7" x2="104" y2="24.7" ${NS}/><text class="i-txt i-cap" x="6" y="19">regard dominant</text>`; break;
    case 'yeux-oblique': s += `<g transform="rotate(-16 80 45)"><rect class="i-wall" x="-40" y="-40" width="240" height="80"/><rect class="i-ground" x="-40" y="54" width="240" height="90"/><path class="i-line2" d="M-40,66 L200,66 M-40,82 L200,82" ${NS}/>${bust(84, 40, 10)}<line class="i-eyeline" x1="-40" y1="39" x2="200" y2="39" ${NS}/></g><text class="i-txt i-cap" x="6" y="84">l'horizon bascule</text>`; break;
    case 'yeux-hierarchie': s += `<rect class="i-wall" x="0" y="0" width="160" height="90"/><rect class="i-sky" x="10" y="0" width="6" height="90"/><rect class="i-sky" x="144" y="0" width="6" height="90"/>${bust(66, 22, 10)}<line class="i-eyeline" x1="80" y1="21" x2="160" y2="21" ${NS}/>${[8, 22, 34, 48, 60, 74, 88, 102, 114, 128, 142, 154].map((x, i) => { const y = 74 + (i % 3) * 2, r = 4.5 + (i % 2); return `<g class="i-fore"><circle cx="${x}" cy="${y}" r="${r}"/><path d="M${x - r * 2.3},90 Q${x - r * 2.3},${y + r * 1.4} ${x},${y + r * 1.2} Q${x + r * 2.3},${y + r * 1.4} ${x + r * 2.3},90 Z"/></g>`; }).join('')}<text class="i-txt i-cap" x="84" y="36">tous lèvent les yeux</text>`; break;
    case 'yeux-basse': s += `<rect class="i-wall" x="0" y="0" width="160" height="90"/><rect class="i-wall2" x="30" y="0" width="100" height="90"/>${bust(80, 68, 9)}<line class="i-eyeline" x1="4" y1="67" x2="156" y2="67" ${NS}/><text class="i-txt i-cap" x="36" y="14">tout ce poids au-dessus</text>`; break;
    case 'yeux-bascule': s += `<rect class="i-wall" x="0" y="0" width="160" height="90"/><rect class="i-ground" x="0" y="74" width="160" height="16"/><g opacity=".35">${bust(30, 54, 8, 1)}</g><line class="i-dash" x1="4" y1="53" x2="70" y2="53" ${NS}/>${bust(58, 28, 8, 1, 'i-fig')}<line class="i-eyeline" x1="4" y1="27" x2="72" y2="27" ${NS}/>${arrowDefs(id)}${arrow('M40,48 Q44,34 48,30', id)}${bust(114, 42, 8, -1)}<line class="i-eyeline" x1="92" y1="41" x2="156" y2="41" ${NS}/><text class="i-txt i-cap" x="92" y="84">il se lève : il domine</text>`; break;
    case 'yeux-raccord': s += `<rect class="i-wall" x="0" y="0" width="160" height="90"/><rect class="i-sky" x="4" y="16" width="72" height="44"/><rect class="i-sky" x="84" y="16" width="72" height="44"/>${bust(28, 34, 7, 1)}${bust(132, 38, 7, -1)}<rect class="i-frame2" x="4" y="16" width="72" height="44" ${NS}/><rect class="i-frame2" x="84" y="16" width="72" height="44" ${NS}/>${arrowDefs(id)}<path class="i-dash" d="M40,34 L70,38" ${NS}/><path class="i-dash" d="M120,38 L90,34" ${NS}/><text class="i-txt i-cap" x="6" y="12">champ</text><text class="i-txt i-cap" x="86" y="12">contrechamp</text><text class="i-txt i-cap" x="6" y="72">A regarde à droite, un peu vers le bas</text><text class="i-txt i-cap" x="6" y="80">B regarde à gauche, un peu vers le haut</text>`; break;
    case 'yeux-convergents': s += `<rect class="i-wall" x="0" y="0" width="160" height="90"/><rect class="i-ground" x="0" y="66" width="160" height="24"/><rect class="i-door" x="72" y="12" width="16" height="30"/><rect class="i-sky" x="74" y="14" width="12" height="28"/><circle class="i-accf" cx="80" cy="28" r="2"/>${[[22, 54, 7, 1], [46, 62, 6, 1], [118, 60, 6, -1], [140, 52, 7, -1]].map(([x, y, r, d]) => `<line class="i-dash" x1="${x + d * r * 0.5}" y1="${y - r * 0.1}" x2="80" y2="28" ${NS}/>${bust(x, y, r, d)}`).join('')}<text class="i-txt i-cap" x="94" y="16">tous regardent ici</text>`; break;
    case 'yeux-divergents': s += `<rect class="i-wall" x="0" y="0" width="160" height="90"/><rect class="i-ground" x="0" y="70" width="160" height="20"/><rect class="i-wall2" x="78" y="0" width="4" height="70"/>${bust(58, 40, 10, -1)}${bust(102, 42, 10, 1)}${arrowDefs(id)}${arrow('M44,38 L10,38', id)}${arrow('M116,40 L150,40', id)}<text class="i-txt i-cap" x="6" y="12">même cadre, regards opposés</text>`; break;
